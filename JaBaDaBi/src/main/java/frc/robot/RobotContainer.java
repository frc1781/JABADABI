...
import frc.robot.commands.swervedrive.commands.Unjam;
...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    copilotXbox.leftTrigger().whileTrue(shooter.shoot(() -> 50));
    copilotXbox.rightBumper().whileTrue(new Shoot(loader, intake, conveyor, shooter, deploy, () -> 50));
  ...
  public void disabledRunningLights() {
    if (isRed()) {
      CommandScheduler.getInstance().schedule(
          lights.set(new NeonLights.Pattern[] { NeonLights.Pattern.RED, NeonLights.Pattern.TRAVEL }));
    } else {
      CommandScheduler.getInstance().schedule(
